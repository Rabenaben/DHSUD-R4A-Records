...
<div class="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0" x-data="{
    ...
    <div class="fixed inset-0 bg-gray-500 opacity-75 transition-opacity" x-show="show"
        ...
        x-transition:leave-start="opacity-75" x-transition:leave-end="opacity-0" x-on:click="show = false"></div>

    <!-- Modal content -->
    <div class="fixed inset-0 z-50 px-4 py-6 sm:px-0" x-show="show">
        <div class="{{ $maxWidth }} absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-lg bg-white shadow-xl transition-all sm:w-full"
